...
**Status:** The Python/SQLite walking skeleton and stage two under projector "1" are implemented. Broader governance subsystems remain unbuilt; the phase plan in §12 is not a completion checklist. Current stage scope is governed by [ADR 0023](../decisions/0023-stage-two-contract.md); the projector "2" commitment/storage contract is in [ADR 0025](../decisions/0025-incremental-result-commitment.md). Accepted ADRs supersede this document where they conflict.

**Last update (Sep 13 2026):** Added invariant-level notice of ADR 0028's proposed restoration-witness exception and corrected decision navigation. ADR 0028 remains Proposed; these notices do not ratify or implement it. The personality-emergence layer and drift monitor remain unspecified (§13).

**Proposed Dream clarification (Sep 14 2026):** [ADR 0029](../decisions/0029-dream-emission-semantics.md) records a distinct emission type and the Layer A event-domain input restriction. The notices below are conditional on its ratification, in the same sense as the ADR 0028 notices; neither draft supplies present implementation authority.
...
**Proposed Dream qualification:** Under [ADR 0029](../decisions/0029-dream-emission-semantics.md), a Dream emission never transitions into evidence under any gate. Independently accepted acquisition produces a separate observation; it does not convert the emission. Retained emissions remain process history outside Layer A, with their durable recording contract unresolved. This qualification awaits ratification.
...
1. **Append-only Reality.** Layer A is append-only, enforced by the storage engine (§1), not by discipline alone. Synthesis may never mutate it. All change is a new appended event; correction supersedes, never edits.
    - **Proposed restoration-witness qualification:** [ADR 0028 §4.2](../decisions/0028-redaction-and-crypto-shredding.md#42-failure-states-and-restoration) requires separately durable restoration-security state. Thus "all change" remains the rule for Layer A truth, not a claim that witness progress is itself only a ledger append. The witness cannot rewrite events, create world evidence or replace a committed erasure authorization. This qualification is conditional on ratification; it grants no present implementation authority.
...
3. **No Silent Promotion.** No record moves toward VERIFIED without an independent signal. Demotion may fire on internal consistency alone; promotion may not.
    - **Proposed type boundary:** [ADR 0029 §2](../decisions/0029-dream-emission-semantics.md#2-reducer-input-algebra-and-permanent-exclusion) excludes DreamEmission from evidence permanently, even with an independent signal. It is not a ClaimCandidate or observation awaiting promotion. Any acquired observation is a separate event. This boundary is conditional on ratification.
...
5. **Origin is Immutable.** Every *event* carries an immutable origin type. A derived statement can never become a stored fact by mutation — only superseded by a *new event* with a different origin.
    - **Proposed recall grounding:** [ADR 0029 §1](../decisions/0029-dream-emission-semantics.md#1-distinct-type-and-inseparable-origin) requires origin to be inseparable from recall: a recalled emission carries its emission origin, and a representation losing that origin is non-conformant. A new observation does not relabel the old emission. This qualification awaits ratification.
...
7. **Reason Freely, Remember Conservatively.** Reasoning is ephemeral by default; memory is durable by exception. Nothing enters Layer A unless an **exception** fires — exactly the oracle set — **never** the strength or repetition of the reasoning itself.
    - **Proposed Dream limit:** [ADR 0029 §§2–3](../decisions/0029-dream-emission-semantics.md#3-emissions-remain-process-history) supplies no exception admitting emissions to Layer A. Retained emissions are process history outside it; their append/mutation policy, durable authority and recovery remain unresolved. This is conditional on ratification and supplies no new Invariant 8 exception.
8. **Commit-Point Sovereignty.** A Layer A event append is the *only* durable commit. Resolved View, queues, indexes, caches, audit state, and verification debt are reconstructible derivatives. **No derived artifact may be the sole copy** of a fact, correction, obligation, or queue item.
    - **Proposed, narrowly scoped exception:** [ADR 0028 §§4.2 and 10](../decisions/0028-redaction-and-crypto-shredding.md#42-failure-states-and-restoration) requires a rollback-resistant restoration witness outside the rollback domain of ordinary log/database and key-service backups. Its monotonic checkpoints, pending fences, authenticated destruction evidence and non-resurrection state cannot be reconstructed by replaying an arbitrary restored snapshot. Its durable security updates are the specific exception to "only durable commit" and reconstructibility. Layer A retains authority for which units may be erased and why; the witness preserves restoration safety, not alternative belief truth. This does not exempt ordinary caches, queues, audit state, usage records or other derived artifacts. ADR 0028 is still Proposed: the exception is recorded for ratification review, not in force.
9. **Projection Determinism.** A Resolved View is `project(event_log, as_of_time, projector_version)`. Conflict resolution, decay, entity-link treatment, and tie-breaks must be deterministic, time-aware, and versioned.
    - **Proposed input-algebra boundary:** [ADR 0029 §§2/6](../decisions/0029-dream-emission-semantics.md#2-reducer-input-algebra-and-permanent-exclusion) admits only Layer A event references to the reducer's input algebra; Dream emissions never participate as reduction inputs, including during belief replay. This preserves explicit time/version parameters, ADR 0014's snapshot/envelope/payload contract and ADR 0025's complete dependency contents. It does not select a bare-ID API or permit mutable process state as a hidden input. The boundary awaits ratification; separate process-history replay and verification remain open.
    - **Proposed restoration boundary:** Under [ADR 0028 §4.2](../decisions/0028-redaction-and-crypto-shredding.md#42-failure-states-and-restoration), deterministic replay of an old snapshot is not sufficient authorization to restore protected access. The independent witness first establishes whether the restored history is current and safe; missing or inconsistent proof blocks recovery/disclosure. It is not a new world-evidence input or permission for nondeterministic reduction. Equal complete replay inputs still produce equal logical results; witness availability governs whether protected recovery may proceed. This boundary awaits ratification.
...
14. **Erasure Boundary — envelope/payload split.** The constitutional guarantee is that nothing is *silently* rewritten. Envelopes retain event identity, order and payload/hash-chain commitments; payload content is stored separately. **Target (unbuilt):** PII-free envelopes with opaque actor IDs and no private free text; payload encryption at write, bound to canonical subjects (§11); privileged, oracle-gated crypto-shredding that destroys keys rather than silently rewriting history. Layer B may propose redaction, never execute it; the target credential fast path requires post-hoc gating. The target preserves auditable envelope commitments after authorized erasure, not the ability to re-verify erased plaintext.
    - **Shipped status:** Under [ADR 0002](../decisions/0002-payload-stored-plaintext-in-v0.md), payloads are plaintext canonical JSON in the column named `ciphertext`; no encryption, key custody, erasure executor or typed REDACTED sentinel ships. `integrity.decode_payload` refuses redacted rows; missing payload rows/content fail integrity checks. Envelope hashing alone does not make full read/replay verification survive payload loss. Arbitrary `source.config` contents are not screened for PII, so a PII-free envelope is a target, not an enforced guarantee. [ADR 0028](../decisions/0028-redaction-and-crypto-shredding.md) remains **Proposed / Implementation: None**: its data classes and unresolved public/protected representation contracts do not establish a shipped privacy boundary.
    - **Proposed non-resurrection requirement:** [ADR 0028 §4.2](../decisions/0028-redaction-and-crypto-shredding.md#42-failure-states-and-restoration) qualifies restoration after erasure: an intact old envelope chain or database/key backup cannot establish that access remains permitted. Independently retained witness evidence must prevent completed destruction from being rolled back before any key unwrap or protected disclosure. If that evidence is unavailable, recovery blocks; it must not recreate an empty witness from the old snapshot. This is the specific proposed restoration safeguard, not ratification of the remaining erasure protocol or a replacement for actual key destruction.
...
**Proposed Dream type:** [ADR 0029](../decisions/0029-dream-emission-semantics.md) distinguishes DreamEmission from ClaimCandidate, observation and evidence. Retention and recall preserve that type and origin. The reducer's authoritative input domain remains Layer A events; not every event is property evidence. This is conditional on ratification and changes no shipped projector or storage representation.

**Resolved View — materialized deterministic projection.** **Decided:** the view is **materialized by a deterministic delta-reducer**, not recomputed by full replay on every read (full replay is O(N) and stalls at scale). Each new Layer A event is folded incrementally into the materialized state, which carries a **version hash**; the stale-with-warning read requirement is described in §8. **The recovery-only replay rule is subject to [ADR 0012's explicit whole-view evaluation exception](../decisions/0012-whole-view-equality.md#explicit-whole-view-evaluation-exception).** The projection obeys Invariant 9 — deterministic, versioned, computed `as_of` an explicit time. Per Commit-Point Sovereignty, the materialized view is always rebuildable from Layer A and never the sole copy of anything. Materialized `storage.read_belief` results in versions "1"/"2" include a stale label when present; `storage.read_belief_status` also discloses freshness for absent/unpublished beliefs. Projector "0" status reads expose derivable freshness or explicit unknown freshness when a legacy materialization has no recorded publication checkpoint (§8). Its raw `read_belief` return shape remains unchanged.
...
**Proposed Dream qualification:** Under [ADR 0029 §§3–4](../decisions/0029-dream-emission-semantics.md#4-acquisition-produces-a-separate-observation), retained emissions remain process history and independent acquisition supplies a separate observation. Whether that observation records a causal emission link remains open; ADR 0021's constitutive mention/subject association does not decide it. This qualification awaits ratification and does not authorize an emission store.
...
**Dependency hashing — shipped helper; Liver drift detection is unbuilt.** `hashing.dep_hash` sorts the supplied event-hash strings, serializes that list with shared canonical JSON and hashes its UTF-8 bytes. It neither discovers dependency events nor updates support membership. Appending a correction leaves an earlier event's hash unchanged; the dependency digest changes only when the caller supplies different hash material. **Target (unbuilt):** a Liver consumer maintains the relevant dependency set, detects support/supersession changes, compares commitments and schedules only necessary re-derivation under `idle_compute_budget`. Automatic drift detection, persistent lookup costs and the resulting scaling benefit are not implemented or established by the helper. Queue-scoring defaults are specified in V0 §2; the consumer and cascade budget remain unbuilt/open (§13).
...
For the accepted usage/evidence boundary, see [ADR 0026](../decisions/0026-usage-is-not-evidence.md).

**Proposed Dream qualification:** [ADR 0029](../decisions/0029-dream-emission-semantics.md) extends that boundary consistently: emissions never become evidence under the oracle/write-exception rules above. The activation scorer and Dream salience receive no causal-provenance handle. Retrieval feedback, pre-governance Generative Flux/raw emission count/Inquiry Yield, preserved open inquiry and Spleen's operational Dream-yield definition remain unresolved. These proposed constraints await ratification; they do not define or implement those consumers.
...
**Proposed Dream exclusion:** Under [ADR 0029 §§2/5](../decisions/0029-dream-emission-semantics.md#5-causal-asymmetry-in-process-provenance), the re-entrant Dream example here and in §2 is prohibited emission-to-evidence conversion, not permission to admit emissions after collapsing their source class. Recorded process provenance may prevent overcounting, never create support. This clarification is conditional on ratification; the deterministic recording/verification contract for counting restrictions remains open.
...
**Two roots of state can't commit atomically (crash between key destruction and completion).** Chain (SQLite) and keystore are separate systems; a crash between destroying a key and appending `redaction_completed` lands in an uncovered state — a single-node design smuggling a distributed-systems problem. → **target recovery (unbuilt):** use the chain as a write-ahead log of intent: startup scans for `*_requested` without a matching `*_completed` and rolls forward; keystore operations are idempotent so replay is safe. Requires crash-point traces (kill between every adjacent step pair, on paper) before trust, not just the happy-path trace. No keystore or erasure startup scan ships; see §5 and Proposed [ADR 0028](../decisions/0028-redaction-and-crypto-shredding.md).
...
**Permanently unverifiable — target (unbuilt).** The intended treatment of `verifiability = subjective | structurally_unverifiable` is explicit flagging, decay and no promotion. **Shipped status:** observed-origin value-setting events receive `verified` from `projection._state_for_origin`, following [ADR 0001](../decisions/0001-observation-recorded-resolves-to-verified.md) and, for legacy corrections, [ADR 0004](../decisions/0004-correction-appended-supersedes-via-superseding-events.md). That assignment does not branch on verifiability; an otherwise accepted observed-origin claim can therefore be `verified` with either label. Other origin mappings, decay and the broader promotion guard remain unbuilt. The target sentence is not a shipped verifiability-based admission or promotion rule.

### Erasure execution (two-phase, provenanced)

**Target (unbuilt):** the sequence and evidentiary treatment below describe intended erasure behavior, not a shipped write or replay path. Current stores retain plaintext payloads under [ADR 0002](../decisions/0002-payload-stored-plaintext-in-v0.md); redacted rows raise `NotImplementedError`, missing payload rows/content raise `IntegrityError`, and no typed REDACTED sentinel exists. Retained envelope commitments do not make erased plaintext verifiable. [ADR 0028](../decisions/0028-redaction-and-crypto-shredding.md), including its public/private data-class and replay contracts, remains **Proposed / Implementation: None**. Arbitrary source configuration is not currently guaranteed PII-free. The following target must be read subject to that unratified protocol work, not as authorization to implement it.
...
- **Routine read:** uses the materialized Resolved View (§1), without an oracle, Liver work or Layer A replay. Projectors "0"/"1" decode the stored belief and its already-materialized support collections; they do not rebuild support from the log or expand separate Merkle collections. Decoding still costs proportional to the returned content. Under projector "2", [ADR 0025 §§3/5](../decisions/0025-incremental-result-commitment.md#3-collection-keys-and-coverage) requires full logical belief reads to reconstruct the accumulated `claim_candidates`, `event_dependencies` and `identity_records` collections from stored tree nodes. That enumeration is part of a full read, not restricted to the Inspector. Named-candidate reads remain indexed and do not require expanding all belief collections; this does not change incremental publication or authorize replay on ordinary reads.
...
**Proposed causal boundary:** [ADR 0029 §§5/7](../decisions/0029-dream-emission-semantics.md#5-causal-asymmetry-in-process-provenance) permits recorded process provenance to prevent overcounting of support, never to create support. It provides no causal-provenance handle to activation scoring or Dream salience and no mutable-store input to belief reduction. This qualification awaits ratification; it neither defines emission storage by analogy with process traces nor settles the operational feedback and yield metrics.
...
- **Stale-projection warning** when a read is served against a not-yet-caught-up view (§1). Versions "1"/"2" label present materialized beliefs; their status APIs also disclose absent/unpublished results. `storage.read_belief_status(..., projector_version="0")` and `storage.read_projection_status(..., projector_version="0")` expose legacy freshness where applied progress is recorded. An existing legacy materialization without that checkpoint returns `stale=None`, `freshness_state="unknown"` and a reason; it is not reported fresh or inferred from timestamps/hash equality. Raw projector-"0" `read_belief` retains its original unlabeled shape. [ADR 0014 §8](../decisions/0014-cross-belief-reducer-and-hash-lineage.md#8-append-freshness-and-derived-progress) governs the applied-progress versus append-position comparison implemented by `projection.is_stale`.
...
**Target erasure-key binding (unbuilt):** bind keys to canonical subjects, not mentions. Per-subject crypto-shredding keys (§5) are meaningless pinned to a pre-resolution mention — a key tied to a mention that later merges into a different canonical entity has no clear owner. Keys resolve post-merge/split against the current canonical identity. Current payloads have no encryption/key binding under [ADR 0002](../decisions/0002-payload-stored-plaintext-in-v0.md). The proposed historical/effective binding and independent-unit custody contract in [ADR 0028](../decisions/0028-redaction-and-crypto-shredding.md) remains unratified; this original target rationale does not select or implement it.

**v2 full graph (target; unbuilt):** lifecycle `mention → candidate → canonical → merged → split`; two confidence tracks (claim vs. entity-link, latter a ceiling); merges are events not mutations; resolver never auto-collapses on similarity; offline merges propose-only. The identity-contradiction veto concerns evidence against the subjects being the same entity, not merely disagreement in their property values. [ADR 0013 §4](../decisions/0013-cross-belief-identity-semantics.md#4-conflicting-values) requires property conflicts to survive a merge as explicit candidates with their evidence/provenance and no authoritative scalar; conflicting property values alone are not a blanket merge veto. Merge handlers remain unimplemented under [ADR 0023](../decisions/0023-stage-two-contract.md).
...
- Event serialization, ordering, idempotency, and hash chaining ship in `src/nyx/hashing.py`, `events.py`, `projection.py`, and `storage.py`. `integrity.verified_log` verifies envelope/payload hashes and predecessor links over the full ordered input log before `as_of` filtering in `projection.project` and `project_snapshot` (the snapshot-projector path in `project` delegates to `project_snapshot`). `nyx verify --db PATH` exposes full-store integrity verification; its result is not a reducer-correctness audit. A standalone replay-verifier subsystem remains unbuilt. Applied publication checkpoints ship for freshness; resumable recovery checkpoints remain unbuilt.
- Reducer/evaluation contracts are governed by [ADRs 0010](../decisions/0010-projection-parameters.md), [0012](../decisions/0012-whole-view-equality.md), and [0014](../decisions/0014-cross-belief-reducer-and-hash-lineage.md); stage-two implementation ships. Database compatibility policy is governed by [ADR 0011](../decisions/0011-database-schema-versioning.md), with current version selection in [ADR 0025 §5](../decisions/0025-incremental-result-commitment.md#5-incremental-representation-and-atomic-publication), superseding ADR 0017's selection. Decay evaluation remains deferred. The legacy `occurred_at`/rowid value tie-break is projector "0" only; see [ADR 0024](../decisions/0024-no-authoritative-head.md) for projector "1" and [ADR 0025](../decisions/0025-incremental-result-commitment.md) for projector "2".
...
- **Content-addressed dependency hash** — `hashing.dep_hash` hashes the caller-supplied event hashes (§3). Using it to bound Liver re-derivation or process-trace matching is a target; those consumers and automatic dependency discovery remain unbuilt.
- **Derived statement** — a Layer B generation. Transient; enters Layer A only via a new oracle-confirmed event.
- **DreamEmission (proposed)** — under [ADR 0029](../decisions/0029-dream-emission-semantics.md), a distinct semantic type, never a ClaimCandidate, observation or evidence. Origin is inseparable from recall. Independent acquisition creates a separate observation; the emission has no promotion path. This qualifies the derived-statement entry only upon ratification; recording/replay mechanics remain open.
...
- **Envelope / payload** — the shipped envelope commits to separately stored plaintext payload content. **Target (unbuilt):** authorized crypto-shredding preserves envelope commitments while making content unavailable. Current full read/replay verification refuses redacted or missing payloads; see Invariant 14, §5 and Proposed [ADR 0028](../decisions/0028-redaction-and-crypto-shredding.md).
